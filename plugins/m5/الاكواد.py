...
# استدعاء البوت من الملف الرئيسي ليعمل الاستقبال هنا
try:
...
async def add_key(event):
    ...
    db = load_keys()
    serial = len(db) + 1
    db[str(serial)] = {"code": code, "days": days, "status": "available"}
    save_keys(db)
...
async def list_keys(event):
    ...
    msg = "📋 **قائمة المفاتيح:**\n"
    for s, d in db.items():
        msg += f"{s} - `{d['code']}` | {d['days']}يوم | {'✅' if d['status']=='available' else '❌'}\n"
